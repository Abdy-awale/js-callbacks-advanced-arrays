...
let fullName = [];
const runnerFullName = runners.forEach(function (runner) {
...
let allCaps = [];
console.log(allCaps);
const FirstNameUpperCase = runners.map((e) => {
...
let largeShirts = [];
const largeShirt = runners.filter((e) => {
	if (e.shirt_size === 'L') {
		largeShirts.push(e);
	}
});
...
let ticketPriceTotals = 0;
const ticketPriceTotal = runners.reduce((acc, curr) => {
	return acc + curr.donation;
}, 0);

// const ticketPriceTotal = runners.filter((acc, cv) => {
